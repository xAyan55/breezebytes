...
const ServerList = () => {
  ...
  const filtered = servers.filter((s) => {
    ...
    const matchesFilter =
      filter === 'all' ||
      (filter === 'online' && s.status === 'running') ||
  ...
  return (
    <div className="flex flex-col gap-6">
      <BreezePageHeader
        ...
        title="My Servers"
        description="View and manage all of your Minecraft server instances."
        icon={Server}
      >
      ...
      {/* ===== Toolbar ===== */}
      <BreezeCard className="p-4 flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between">
        {/* Search */}
        <div className="relative flex-1 max-w-sm">
          <Search
            size={16}
            className="absolute left-3 top-1/2 -translate-y-1/2 text-p5/50 pointer-events-none"
          />
          <input
            type="text"
            ...
            className="w-full pl-9 pr-4 py-2 bg-s1 border-2 border-s3 rounded-2xl text-sm text-p4 placeholder:text-p5/40 focus:outline-none focus:border-s4 transition-all duration-500"
          />
        </div>

        {/* Filters */}
        <div className="flex items-center gap-1 p-1 bg-s1 border-2 border-s3 rounded-2xl">
          {filters.map((f) => (
            <button
              key={f.id}
              onClick={() => setFilter(f.id)}
              className={clsx(
                'px-4 py-1.5 rounded-xl text-xs font-semibold transition-all duration-500',
                filter === f.id
                  ? 'g4 text-p1 border border-s4/30'
            ...
            <Link key={s.id} to={`/panel/servers/${s.id}/console`}>
              <BreezeCard hover className="relative p-5 flex flex-col justify-between gap-4 h-full overflow-hidden group">
                ...
                <div className="relative z-10 flex items-center gap-3 pt-3 border-t-2 border-s3/80">
                  ...
                  <span className="small-2 text-p5 capitalize ml-auto">
