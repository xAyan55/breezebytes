...
const AdminUsers = () => {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      showNotification('error', `User creation failed: ${err.message}`);
    }
  };
  ...
  const handleSuspendToggle = async (user) => {
    try {
      ...
      showNotification('success', user.is_suspended ? 'User unsuspended.' : 'User suspended.');
      fetchUsers();
    } catch (err) {
      showNotification('error', err.message);
    }
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-s3/60">
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-s5/30 transition-colors duration-300">
                    <td className="py-3 px-4 font-bold text-p4 flex items-center gap-2.5">
                      <div className="size-7 rounded-full bg-s4/20 border border-s4/30 flex items-center justify-center text-xs font-bold text-p1 font-sans">
                        {u.username.charAt(0).toUpperCase()}
                      </div>
                      <span>{u.username}</span>
                    </td>
                    <td className="py-3 px-4 text-p5 font-mono">{u.email}</td>
                    <td className="py-3 px-4">
                      <select
                        value={u.role}
                        onChange={(e) => handleRoleChange(u.id, e.target.value)}
                        className="bg-s1 border border-s3 rounded-xl px-2.5 py-1 text-xs text-p4 uppercase font-semibold focus:outline-none focus:border-s4 transition-colors"
                      >
